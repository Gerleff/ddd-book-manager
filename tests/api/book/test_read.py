...
async def test_concurent(book_from_db: Book, another_book_from_db: Book, client: TestClient):
    ...
    result = await asyncio.gather(
        *[_request(model) for model in (book_from_db.id, another_book_from_db.id, *([uuid.uuid4()] * 100))] * 10
    )
